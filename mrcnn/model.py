...
from mrcnn import custom_layers
...
class NormalizeBoxes(Layer):
    def call(self, inputs):
        input_image, input_gt_boxes = inputs
        return norm_boxes_graph(input_gt_boxes, K.shape(input_image)[1:3])
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # gt_boxes = NormalizeBoxes()([input_image, input_gt_boxes])
        ...
        if mode == "training":
            anchors = self.get_anchors(config.IMAGE_SHAPE)
            anchors = custom_layers.AnchorsLayer(anchors, config.BATCH_SIZE)(input_image)
        else:
            anchors = input_anchors
        ...
        if config.GPU_COUNT > 1:
            from mrcnn.parallel_model_v2 import ParallelModel
